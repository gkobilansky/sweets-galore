Here's a drop-in **PRD.md** you can paste into `sweets-galore/` and point Claude Code at.

```markdown
# SWEETS GALORE — Product Requirements Document (PRD)
...
* `localStorage` keys:
  ...
  * `sweets-galore:highscore`
  * `sweets-galore:settings:sound`
  * `sweets-galore:settings:reduceMotion`
...
* Manifest:
  ...
  * `name: "Sweets Galore"`, `short_name: "Sweets Galore"`
