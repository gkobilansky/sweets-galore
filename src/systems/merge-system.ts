...
type MergeResolutionMode = 'standard' | 'cap-piece';
...
export interface CapPieceClearEvent {
  tier: TierConfig;
...
export class MergeSystem {
  ...
  public onCapPieceClear?: (event: CapPieceClearEvent) => void;
  ...
  private handleCollision(piece1: GamePiece, piece2: GamePiece): void {
    ...
    if (canStandardMerge) {
      mode = 'standard';
    } else if (this.shouldHandleCapPieceClear(piece1.tier.id)) {
      mode = 'cap-piece';
    } else {
      return;
    }
    ...
    const label = mode === 'cap-piece' ? 'Cap piece clear' : 'Merge';
  ...
  public update(_deltaTime: number): void {
    ...
    for (const { key, candidate } of candidatesToProcess) {
      ...
      if (candidate.mode === 'cap-piece') {
        this.executeCapPieceDetonation(candidate);
      } else {
  ...
  private executeCapPieceDetonation(candidate: MergeCandidate): void {
    const { piece1, piece2 } = candidate;
    const clearX = (piece1.body.position.x + piece2.body.position.x) / 2;
    const clearY = (piece1.body.position.y + piece2.body.position.y) / 2;

    console.log(`Executing cap piece clear at (${Math.round(clearX)}, ${Math.round(clearY)})`);
    ...
    const baseScore = GAME_CONFIG.capPieceClearScore;
    const finalScore = Math.floor(baseScore * this.comboState.multiplier);

    if (this.onCapPieceClear) {
      this.onCapPieceClear({
        tier: piece1.tier,
  ...
  private shouldHandleCapPieceClear(tierId: number): boolean {
    return tierId === GAME_CONFIG.capPieceTierId;
  }
