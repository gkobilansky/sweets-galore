...
import { MergeSystem, type CapPieceClearEvent } from "../../systems/merge-system";
...
export class GameScene extends PixiContainer implements SceneInterface {
    ...
    private capPieceShockwaves: { node: PixiGraphics; life: number; initialLife: number; minScale: number; maxScale: number }[] = [];
    ...
    private setupSystems(): void {
        ...
        this.mergeSystem.onCapPieceClear = (event) => {
            this.handleCapPieceClear(event);
        };
    ...
    private setupUI(): void {
        ...
        this.floorRect.fill({ color: 0xADD8E6 }); // Pastel blue color for floor
    ...
    private handleCapPieceClear(event: CapPieceClearEvent): void {
        this.score += event.score;
        this.updateScoreDisplay();
        AudioManager.playMerge(event.tier.id);
        AudioManager.playCapPieceClear();

        this.spawnScreenFlash(0xfff4cf, 0.35);
        this.spawnCapPieceClearEffect(event.position, event.tier);
        this.spawnCapPieceCallout(event.position, event.tier, event.score, event.comboMultiplier);
    }
    ...
    private spawnCapPieceClearEffect(position: { x: number; y: number }, tier: TierConfig): void {
        ...
        this.trackCapPieceShockwave(outerWave, 0.9, 0.35, 2.8);
        ...
        this.trackCapPieceShockwave(innerBurst, 0.4, 0.1, 1.6);
    }

    private trackCapPieceShockwave(node: PixiGraphics, duration: number, minScale: number, maxScale: number): void {
        this.capPieceShockwaves.push({
            node,
            life: duration,
    ...
    private spawnCapPieceCallout(position: { x: number; y: number }, tier: TierConfig, score: number, comboMultiplier: number): void {
        const multiplierSuffix = comboMultiplier > 1 ? ` (x${comboMultiplier.toFixed(1)} combo)` : '';
        const text = new PixiText({
            text: `SWEET EXPLOSION\n+${score}${multiplierSuffix}`,
            style: {
    ...
    update(framesPassed: number): void {
        ...
        if (this.capPieceCallouts.length) {
            for (let i = this.capPieceCallouts.length - 1; i >= 0; i--) {
                const entry = this.capPieceCallouts[i];
                entry.life -= dt;
                ...
                if (entry.life <= 0) {
                    ...
                    entry.node.destroy();
                    this.capPieceCallouts.splice(i, 1);
                }
            }
        }

        if (this.capPieceShockwaves.length) {
            for (let i = this.capPieceShockwaves.length - 1; i >= 0; i--) {
                const wave = this.capPieceShockwaves[i];
                wave.life -= dt;
                ...
                if (wave.life <= 0) {
                    ...
                    wave.node.destroy();
                    this.capPieceShockwaves.splice(i, 1);
                }
            }
        }
    ...
    destroy(): void {
        ...
        this.mergeEffects = [];
        for (const wave of this.capPieceShockwaves) {
            this.removeChild(wave.node);
            wave.node.destroy();
        }
        this.capPieceShockwaves = [];
        for (const callout of this.capPieceCallouts) {
            this.removeChild(callout.node);
            callout.node.destroy();
        }
        this.capPieceCallouts = [];
